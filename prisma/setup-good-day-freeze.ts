...
async function main() {
  console.log('Memulai pengaturan Kopi Good Day Freeze & Es Kopi...');
  ...
  // 2. Ambil semua produk Good Day Freeze sachet (non-olahan)
  ...
  console.log(`Ditemukan ${sachetProducts.length} produk Good Day Freeze sachet di database.`);

  for (const sachet of sachetProducts) {
    // A. Update harga sachet (modal 2200, jual 3000)
    ...
    const escSku = sachet.sku ? `ESC-${sachet.sku}` : `ESC-GDF-${Date.now()}`;
  ...
  console.log('Semua data Kopi Good Day Freeze & Es Kopi telah dikonfigurasi!');
}
